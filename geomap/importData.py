...
for wochenmarkt in data['wochenmaerkte']:
    ...
    for n in range(numberOfWeeks):
        creation_date = mondayMidnight + datetime.timedelta(days=weekday + numberOfWeeks*7, hours = hours_Start)
        ...
        event.valid_until = mondayMidnight + datetime.timedelta(days=weekday + numberOfWeeks*7, hours = hours_Start) + datetime.timedelta(hours = hours_End)
        event.save()
